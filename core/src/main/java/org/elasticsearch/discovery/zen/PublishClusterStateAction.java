...
public class PublishClusterStateAction extends AbstractComponent {
    ...
    private void innerPublish(final ClusterChangedEvent clusterChangedEvent, final Set<DiscoveryNode> nodesToPublishTo,
        ...
        for (final DiscoveryNode node : nodesToPublishTo) {
            System.out.println(LocalDateTime.now() + " PublishClusterStateAction -> innerPublish n193, publish to node: " +node+ "," +this.getClass());
    ...
    protected void handleCommitRequest(CommitClusterStateRequest request, final TransportChannel channel) {
        ...
        if (state != null) {
            System.out.println(LocalDateTime.now() + " PublishClusterStateAction -> handleCommitRequest n478:" + "master " + state.nodes().getMasterNode() + " committed version [" + state.version() + "]");
