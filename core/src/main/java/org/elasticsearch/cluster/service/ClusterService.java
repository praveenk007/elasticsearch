...
public class ClusterService extends AbstractLifecycleComponent {
    ...
    public <T> void submitStateUpdateTasks(final String source,
                                           ...
                                           final ClusterStateTaskExecutor<T> executor) {
        System.out.println(LocalDateTime.now() + String.format(" ClusterService -> submitStateUpdateTasks n435,source=【%s】,tasks.size()=%s", source, tasks.size()));
        if (!lifecycle.started()) {
            return;
        }
        if (tasks.isEmpty()) {
    ...
    void runTasks(TaskInputs taskInputs) {
        System.out.println(LocalDateTime.now() + " 开始执行任务 runTasks(taskInputs)，ClusterService -> runTasks n603, taskInputs= " + taskInputs.summary + ", " + this.getClass());
        if (!lifecycle.started()) {
        ...
        if (!previousClusterState.nodes().isLocalNodeElectedMaster() && taskInputs.runOnlyOnMaster()) { //主节点的操作
        ...
        System.out.println("calculating taskOutputs ......");
        TaskOutputs taskOutputs = calculateTaskOutputs(taskInputs, previousClusterState, startTimeNS);
        System.out.println("calculate taskOutputs Done!，下面将发布并提交，taskOutputs=" + taskOutputs.executionResults);
        ...
        if (taskOutputs.clusterStateUnchanged()) {
            ...
            try {
                System.out.println("publish and apply changes ......");
                publishAndApplyChanges(taskInputs, taskOutputs);
                System.out.println("publish and apply changes Done!");
                TimeValue executionTime = TimeValue.timeValueMillis(Math.max(0, TimeValue.nsecToMSec(currentTimeInNanos() - startTimeNS)));
    ...
    private void publishAndApplyChanges(TaskInputs taskInputs, TaskOutputs taskOutputs) {
        ...
        if (nodesDelta.hasChanges() && logger.isInfoEnabled()) {
            ...
            System.out.println("nodes delta short summary:" + summary);
            if (summary.length() > 0) {
        ...
        if (newClusterState.nodes().isLocalNodeElectedMaster()) { //主节点的操作
            logger.debug("publishing cluster state version [{}]", newClusterState.version());
            try {
                System.out.println("call PublishClusterStateAction.publish ......");
                clusterStatePublisher.accept(clusterChangedEvent, ackListener);
                System.out.println("call PublishClusterStateAction.publish Done!");
