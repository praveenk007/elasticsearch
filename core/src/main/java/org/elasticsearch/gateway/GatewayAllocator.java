...
public class GatewayAllocator extends AbstractComponent {
    ...
    protected static void innerAllocatedUnassigned(RoutingAllocation allocation,
        ...
        System.out.println(LocalDateTime.now() + " GatewayAllocator -> primaryShardAllocator.allocateUnassigned(allocation) ...");
        primaryShardAllocator.allocateUnassigned(allocation);
        System.out.println(LocalDateTime.now() + " GatewayAllocator -> primaryShardAllocator.allocateUnassigned(allocation) Done!");

        System.out.println(LocalDateTime.now() + " GatewayAllocator -> replicaShardAllocator.processExistingRecoveries(allocation) ...");
        replicaShardAllocator.processExistingRecoveries(allocation);
        System.out.println(LocalDateTime.now() + " GatewayAllocator -> replicaShardAllocator.processExistingRecoveries(allocation) Done!");

        System.out.println(LocalDateTime.now() + " GatewayAllocator -> replicaShardAllocator.allocateUnassigned(allocation) ...");
        replicaShardAllocator.allocateUnassigned(allocation);
        System.out.println(LocalDateTime.now() + " GatewayAllocator -> replicaShardAllocator.allocateUnassigned(allocation) Done!");
    }
    ...
    class InternalAsyncFetch<T extends BaseNodeResponse> extends AsyncShardFetch<T> {
        ...
        protected void reroute(ShardId shardId, String reason) {
            System.out.println(LocalDateTime.now() + " GatewayAllocator -> reroute n155 " + this.getClass());
    ...
    class InternalPrimaryShardAllocator extends PrimaryShardAllocator {
        ...
        protected AsyncShardFetch.FetchResult<TransportNodesListGatewayStartedShards.NodeGatewayStartedShards> fetchData(ShardRouting shard, RoutingAllocation allocation) {
            System.out.println(LocalDateTime.now() + " GatewayAllocator -> fetchData n163 " + this.getClass());
