...
public abstract class PrimaryShardAllocator extends BaseGatewayShardAllocator {
    ...
    public AllocateUnassignedDecision makeAllocationDecision(final ShardRouting unassignedShard,
                                                             ...
                                                             final Logger logger) {
        System.out.println(LocalDateTime.now() + " PrimaryShardAllocator making AllocationDecision... n120 " + this.getClass());
        if (isResponsibleFor(unassignedShard) == false) {
            System.out.println("sorry not response for this shard allocation!");
        ...
        if (inSyncAllocationIds.isEmpty()) {
            System.out.println("inSyncAllocationIds is empty");
            ...
            System.out.println("inSyncAllocationIds is not empty");
        ...
        if (enoughAllocationsFound == false) {
            System.out.println("enoughAllocationsFound is false");
            if (snapshotRestore) {
                System.out.println("snapshotRestore is true");
                ...
                System.out.println("recoverOnAnyNode is true");
                // let BalancedShardsAllocator take care of allocating this shard
                logger.debug("[{}][{}]: missing local data, recover from any node", unassignedShard.index(), unassignedShard.id());
                return AllocateUnassignedDecision.NOT_TAKEN;
            } else {
                System.out.println("snapshotRestore and recoverOnAnyNode are false");
        ...
        if (nodesToAllocate.yesNodeShards.isEmpty() == false) {
            System.out.println("nodesToAllocate.yesNodeShards.isEmpty() is false");
            ...
            System.out.println("nodesToAllocate.throttleNodeShards.isEmpty() && !nodesToAllocate.noNodeShards.isEmpty() is true");
            ...
            System.out.println("we are throttling this");
        ...
        if (allocation.hasPendingAsyncFetch()) {
            System.out.println("hasPendingAsyncFetch");
            ...
            System.out.println("throttled is true");
            return AllocateUnassignedDecision.throttle(nodeResults);
        } else {
            System.out.println("DECIDERS_NO");
    ...
    protected static NodeShardsResult buildAllocationIdBasedNodeShardsResult(ShardRouting shard, boolean matchAnyShard,
                                                                             ...
                                                                             Logger logger) {
        System.out.println(LocalDateTime.now() + " PrimaryShardAllocator -> buildAllocationIdBasedNodeShardsResult n304 ");
